        ...
        <div class="modal-content bg-white rounded shadow-lg w-11/12 md:w-1/3">
          ...
          <form @submit.prevent="saveFilters()">
            <div class="p-4">
              <!-- CEP -->
              <InputField
                id="postal_code" v-model="formFilters.postal_code" label="CEP"
                wrapper-class="mt-0" @input="formatPostalCode"
              />

              <!-- Rua -->
              <InputField
                id="street_address" v-model="formFilters.street_address" label="Logradouro"
                wrapper-class="mt-0" class="mt-2"
              />

              <!-- Bairro -->
              <InputField
                id="neighborhood" v-model="formFilters.neighborhood" label="Bairro"
                wrapper-class="mt-0" class="mt-2"
              />

              <!-- Cidade -->
              <InputField
                id="city" v-model="formFilters.city" label="Cidade" wrapper-class="mt-0"
                class="mt-2"
              />
              ...
              <div class="mt-5">
                ...
                <RemoveFilterButton
                  label="CEP" :filter-value="filters.postal_code"
                  @remove-filter="clearFilter('postal_code')"
                />
                <RemoveFilterButton
                  label="Logradouro" :filter-value="filters.street_address"
                  @remove-filter="clearFilter('street_address')"
                />
                <RemoveFilterButton
                  label="Bairro" :filter-value="filters.neighborhood"
                  @remove-filter="clearFilter('neighborhood')"
                />
                <RemoveFilterButton
                  label="Cidade" :filter-value="filters.city"
